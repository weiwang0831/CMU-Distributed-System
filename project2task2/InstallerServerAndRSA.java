...
class Connection extends Thread {
    ...
    String title;
    String response;
    BigInteger d = new BigInteger("27556299471488886461385595625864010987843453632898042791765250739361821607525");
    ...
    public void run() {
        try {
            ...
            if (checkASCII(decryptString) == false) {
                System.out.println("Got visit: " + InstallerServerAndRSA.count + " Illegal symmetric key used. This may be an attack.");
                clientSocket.close();
                ...
                if (InstallerServerAndRSA.installerInfo.get(userID) == null) {
                    response = "Illegal ID or Password";
                    System.out.println("Visit: " + InstallerServerAndRSA.count + " from " + userID + ", " + "\nIllegal ID or Password");
                    out.write(response.getBytes());
                    //check if the password enter, after hash with salt value, is equal to what stored in PasswordHash Class
                } else if (!(PasswordHash.hash(InstallerServerAndRSA.installerInfo.get(userID)[0] + passwd)).equals(InstallerServerAndRSA.installerInfo.get(userID)[1])) {
                    title = InstallerServerAndRSA.installerInfo.get(userID)[2];
                    response = "Illegal ID or Password";
                    System.out.println("Visit: " + InstallerServerAndRSA.count + " from " + userID + ", " + title + "\nIllegal ID or Password");
                    out.write(response.getBytes());
                } else if (!"Barack".equals(userID) && InstallerServerAndRSA.sensorInfo.get(sensorID) != null) {
                    ...
                    if (InstallerServerAndRSA.sensorInfo.get(sensorID) == null) {
                        System.out.println("Visit: " + InstallerServerAndRSA.count + " -> "
                    ...
                    InstallerServerAndRSA.sensorInfo.put(sensorID, location);

                    //generate KML file
                    String deskTopLocation = System.getProperty("user.home") + "/Desktop/Sensors.kml";
                    String KMLString = getKML(InstallerServerAndRSA.sensorInfo);
